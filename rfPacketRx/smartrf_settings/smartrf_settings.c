...
// The applied template is compatible with CC13x0 SDK version 2.10.xx.xx or
// newer. Device: CC1310 Rev. B (2.1).
//
//*********************************************************************************
...
// TX Power: 15 dBm (requires define CCFG_FORCE_VDDR_HH = 1 in ccfg.c, see
// CC13xx/CC26xx Technical Reference Manual) Whitening: No whitening
...
RF_Mode RF_prop = {.rfMode = RF_MODE_PROPRIETARY_SUB_1,
...
uint32_t pOverrides[] = {
    ...
    // Synth: Set loop bandwidth after lock to 20 kHz
    (uint32_t)0x7AB80603,
    // Synth: Configure VCO LDO (in ADI1, set VCOLDOCFG=0x9F to use voltage
    // input reference)
    ...
    // Tx: Configure PA ramping setting (0x41). Rx: Set AGC reference level to
    // 0x1A.
...
rfc_CMD_FS_t RF_cmdFs = {.commandNo = 0x0803,
                         .status = 0x0000,
                         .pNextOp =
                             0, // INSERT APPLICABLE POINTER: (uint8_t*)&xxx
                         .startTime = 0x00000000,
                         .startTrigger.triggerType = 0x0,
                         .startTrigger.bEnaCmd = 0x0,
                         .startTrigger.triggerNo = 0x0,
